...
fn main() -> anyhow::Result<()> {
    let model_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("../res/branching_model.oinf");
    ...
    println!("branch condition: {}", cond);
    println!("h[0..100] = {:?}", &h.data[..100.min(h.len())]);

    let trace = exec.trace();
    let out_dir = Path::new(env!("CARGO_MANIFEST_DIR")).join("../examples/rust/out");
    std::fs::create_dir_all(&out_dir)?;
    let out_path = out_dir.join("branching_bad_trace.json");
