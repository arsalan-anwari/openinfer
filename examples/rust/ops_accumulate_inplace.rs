...
fn main() -> anyhow::Result<()> {
    ...
    let g = graph! {
        ...
        block entry {
            ...
            op abs(a_i4) >> abs_i4;
            op relu(a_i4, negative_slope=0.1, clamp_max=2.0) >> relu_i4;
            ...
            op relu(in_i8, negative_slope=0.1, clamp_max=2.0) >> in_i8;
            op relu(in_i16, negative_slope=0.1, clamp_max=2.0) >> in_i16;
            op relu(in_i32, negative_slope=0.1, clamp_max=2.0) >> in_i32;
            op relu(in_i64, negative_slope=0.1, clamp_max=2.0) >> in_i64;
            op relu(in_i4, negative_slope=0.1, clamp_max=2.0) >> in_i4;
            op relu(in_f16, negative_slope=0.1, clamp_max=2.0) >> in_f16;
            op relu(in_bf16, negative_slope=0.1, clamp_max=2.0) >> in_bf16;
            op relu(in_f8, negative_slope=0.1, clamp_max=2.0) >> in_f8;
            op relu(in_f32, negative_slope=0.1, clamp_max=2.0) >> in_f32;
            op relu(in_f64, negative_slope=0.1, clamp_max=2.0) >> in_f64;
    ...
    println!("ops_accumulate_inplace completed on {:?}", device);
