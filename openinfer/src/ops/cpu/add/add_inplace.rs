...
pub fn add_inplace_bf16(a: &mut [BF16], b: &[BF16], thread_id: usize) -> Result<()> {
    if a.len() != b.len() {
        return Err(anyhow!("add inplace shape mismatch"));
    }
    Timer::start(thread_id);
...
pub fn add_inplace_f8(a: &mut [F8E5M2], b: &[F8E5M2], thread_id: usize) -> Result<()> {
    if a.len() != b.len() {
        return Err(anyhow!("add inplace shape mismatch"));
    }
    Timer::start(thread_id);
