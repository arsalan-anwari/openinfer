...
fn add_packed_signed<T: PackedByte + Copy>(
    bits: u8,
    a: &Tensor<T>,
    b: &Tensor<T>,
    out: &mut Tensor<T>,
    ...
    if a.data.len() != b.data.len() || out.data.len() != storage_len {
        return Err(anyhow!("add op packed data length mismatch"));
    }
    Timer::start(thread_id);
...
fn add_packed_unsigned<T: PackedByte + Copy>(
    bits: u8,
    a: &Tensor<T>,
    b: &Tensor<T>,
    out: &mut Tensor<T>,
...
fn add_packed_signed_broadcast<T: PackedByte + Copy>(
    bits: u8,
    a: &Tensor<T>,
    b: &Tensor<T>,
    out: &mut Tensor<T>,
    ...
    if expected != out.shape() {
        return Err(anyhow!(
            "add op packed broadcast shape mismatch: expected {:?}, got {:?}",
            expected,
            out.shape()
        ));
    }
    if !is_contiguous(out.shape(), out.strides()) {
        return Err(anyhow!("add op packed broadcast requires contiguous output"));
    }
    let logical_len = numel(out.shape());
    let storage_len = packed_storage_len(bits, logical_len);
    if out.data.len() != storage_len {
        return Err(anyhow!("add op packed broadcast data length mismatch"));
    }
...
fn add_packed_unsigned_broadcast<T: PackedByte + Copy>(
    bits: u8,
    a: &Tensor<T>,
    b: &Tensor<T>,
    out: &mut Tensor<T>,
...
pub fn add_i8(a: &Tensor<i8>, b: &Tensor<i8>, out: &mut Tensor<i8>, thread_id: usize) -> Result<()> {
    add_elementwise(a, b, out, |x, y| x + y, thread_id)
}

pub fn add_i8_broadcast(
    a: &Tensor<i8>,
    b: &Tensor<i8>,
    out: &mut Tensor<i8>,
    thread_id: usize,
...
pub fn add_f16(
    ...
    add_elementwise(a, b, out, |x, y| F16::from_f32(x.to_f32() + y.to_f32()), thread_id)
}

pub fn add_f16_broadcast(
    a: &Tensor<F16>,
    b: &Tensor<F16>,
    out: &mut Tensor<F16>,
    thread_id: usize,
...
pub fn add_i4(
    ...
    add_packed_signed(4, a, b, out, thread_id)
}

pub fn add_i4_broadcast(
    a: &Tensor<I4>,
    b: &Tensor<I4>,
    out: &mut Tensor<I4>,
    thread_id: usize,
) -> Result<()> {
    add_packed_signed_broadcast(4, a, b, out, thread_id)
}
...
pub fn add_i2(
    ...
    add_packed_signed(2, a, b, out, thread_id)
}

pub fn add_i2_broadcast(
    a: &Tensor<I2>,
    b: &Tensor<I2>,
    out: &mut Tensor<I2>,
    thread_id: usize,
) -> Result<()> {
    add_packed_signed_broadcast(2, a, b, out, thread_id)
}
...
pub fn add_i1(
    ...
    add_packed_signed(1, a, b, out, thread_id)
}

pub fn add_i1_broadcast(
    a: &Tensor<I1>,
    b: &Tensor<I1>,
    out: &mut Tensor<I1>,
    thread_id: usize,
) -> Result<()> {
    add_packed_signed_broadcast(1, a, b, out, thread_id)
}
...
pub fn add_u4(
    ...
    add_packed_unsigned(4, a, b, out, thread_id)
}

pub fn add_u4_broadcast(
    a: &Tensor<U4>,
    b: &Tensor<U4>,
    out: &mut Tensor<U4>,
    thread_id: usize,
) -> Result<()> {
    add_packed_unsigned_broadcast(4, a, b, out, thread_id)
}
...
pub fn add_u2(
    ...
    add_packed_unsigned(2, a, b, out, thread_id)
}

pub fn add_u2_broadcast(
    a: &Tensor<U2>,
    b: &Tensor<U2>,
    out: &mut Tensor<U2>,
    thread_id: usize,
) -> Result<()> {
    add_packed_unsigned_broadcast(2, a, b, out, thread_id)
}
...
pub fn add_u1(
    ...
    add_packed_unsigned(1, a, b, out, thread_id)
}

pub fn add_u1_broadcast(
    a: &Tensor<U1>,
    b: &Tensor<U1>,
    out: &mut Tensor<U1>,
    thread_id: usize,
) -> Result<()> {
    add_packed_unsigned_broadcast(1, a, b, out, thread_id)
}
