...
pub fn add_tensor_i8(a: &Tensor<i8>, b: &Tensor<i8>, thread_id: usize) -> Result<(Vec<i8>, Vec<usize>)> {
    ...
    {
        return Ok((add_i8(&a.data, &b.data, thread_id)?, out_shape));
    }
    let out = add_broadcasted(a, b, &out_shape, |x, y| x + y, thread_id)?;
    Ok((out, out_shape))
}

pub fn add_tensor_i16(a: &Tensor<i16>, b: &Tensor<i16>, thread_id: usize) -> Result<(Vec<i16>, Vec<usize>)> {
    let out_shape = if a.shape() == b.shape() {
        a.shape().to_vec()
    } else {
        broadcast_shapes(a.shape(), b.shape())?
    ...
    if a.shape() == b.shape()
        ...
        && is_contiguous(b.shape(), b.strides())
    {
        return Ok((add_i16(&a.data, &b.data, thread_id)?, out_shape));
    }
    let out = add_broadcasted(a, b, &out_shape, |x, y| x + y, thread_id)?;
    Ok((out, out_shape))
}

pub fn add_tensor_f32(a: &Tensor<f32>, b: &Tensor<f32>, thread_id: usize) -> Result<(Vec<f32>, Vec<usize>)> {
    let out_shape = if a.shape() == b.shape() {
        a.shape().to_vec()
    } else {
        broadcast_shapes(a.shape(), b.shape())?
    ...
    if a.shape() == b.shape()
        ...
        && is_contiguous(b.shape(), b.strides())
    {
        return Ok((add_f32(&a.data, &b.data, thread_id)?, out_shape));
    }
    let out = add_broadcasted(a, b, &out_shape, |x, y| x + y, thread_id)?;
    Ok((out, out_shape))
}

pub fn add_tensor_f64(a: &Tensor<f64>, b: &Tensor<f64>, thread_id: usize) -> Result<(Vec<f64>, Vec<usize>)> {
    let out_shape = if a.shape() == b.shape() {
        a.shape().to_vec()
    } else {
        broadcast_shapes(a.shape(), b.shape())?
    ...
    if a.shape() == b.shape()
        ...
        && is_contiguous(b.shape(), b.strides())
    {
        return Ok((add_f64(&a.data, &b.data, thread_id)?, out_shape));
    }
    let out = add_broadcasted(a, b, &out_shape, |x, y| x + y, thread_id)?;
    Ok((out, out_shape))
}

pub fn add_tensor_f16(a: &Tensor<F16>, b: &Tensor<F16>, thread_id: usize) -> Result<(Vec<F16>, Vec<usize>)> {
    let out_shape = if a.shape() == b.shape() {
        a.shape().to_vec()
    } else {
        broadcast_shapes(a.shape(), b.shape())?
    ...
    if a.shape() == b.shape()
        ...
        && is_contiguous(b.shape(), b.strides())
    {
        return Ok((add_f16(&a.data, &b.data, thread_id)?, out_shape));
    }
    let out = add_broadcasted(a, b, &out_shape, |x, y| F16::from_f32(x.to_f32() + y.to_f32()), thread_id)?;
    Ok((out, out_shape))
}

pub fn add_tensor_bf16(a: &Tensor<BF16>, b: &Tensor<BF16>, thread_id: usize) -> Result<(Vec<BF16>, Vec<usize>)> {
    let out_shape = if a.shape() == b.shape() {
        a.shape().to_vec()
    } else {
        broadcast_shapes(a.shape(), b.shape())?
    ...
    if a.shape() == b.shape()
        ...
        && is_contiguous(b.shape(), b.strides())
    {
        return Ok((add_bf16(&a.data, &b.data, thread_id)?, out_shape));
    }
    let out = add_broadcasted(a, b, &out_shape, |x, y| BF16::from_f32(x.to_f32() + y.to_f32()), thread_id)?;
    Ok((out, out_shape))
}

pub fn add_tensor_f8(a: &Tensor<F8E5M2>, b: &Tensor<F8E5M2>, thread_id: usize) -> Result<(Vec<F8E5M2>, Vec<usize>)> {
    let out_shape = if a.shape() == b.shape() {
        a.shape().to_vec()
    } else {
        broadcast_shapes(a.shape(), b.shape())?
    ...
    if a.shape() == b.shape()
        ...
        && is_contiguous(b.shape(), b.strides())
    {
        return Ok((add_f8(&a.data, &b.data, thread_id)?, out_shape));
    }
    let out = add_broadcasted(a, b, &out_shape, |x, y| F8E5M2::from_f32(x.to_f32() + y.to_f32()), thread_id)?;
    Ok((out, out_shape))
}

pub fn add_tensor_u8(a: &Tensor<u8>, b: &Tensor<u8>, thread_id: usize) -> Result<(Vec<u8>, Vec<usize>)> {
    let out_shape = if a.shape() == b.shape() {
        a.shape().to_vec()
    } else {
        broadcast_shapes(a.shape(), b.shape())?
    ...
    if a.shape() == b.shape()
        ...
        && is_contiguous(b.shape(), b.strides())
    {
        return Ok((add_u8(&a.data, &b.data, thread_id)?, out_shape));
    }
    let out = add_broadcasted(a, b, &out_shape, |x, y| x + y, thread_id)?;
    Ok((out, out_shape))
}

pub fn add_tensor_u16(a: &Tensor<u16>, b: &Tensor<u16>, thread_id: usize) -> Result<(Vec<u16>, Vec<usize>)> {
    let out_shape = if a.shape() == b.shape() {
        a.shape().to_vec()
    } else {
        broadcast_shapes(a.shape(), b.shape())?
    ...
    if a.shape() == b.shape()
        ...
        && is_contiguous(b.shape(), b.strides())
    {
        return Ok((add_u16(&a.data, &b.data, thread_id)?, out_shape));
    }
    let out = add_broadcasted(a, b, &out_shape, |x, y| x + y, thread_id)?;
    Ok((out, out_shape))
}

pub fn add_tensor_i32(a: &Tensor<i32>, b: &Tensor<i32>, thread_id: usize) -> Result<(Vec<i32>, Vec<usize>)> {
    let out_shape = if a.shape() == b.shape() {
        a.shape().to_vec()
    } else {
        broadcast_shapes(a.shape(), b.shape())?
    ...
    if a.shape() == b.shape()
        ...
        && is_contiguous(b.shape(), b.strides())
    {
        return Ok((add_i32(&a.data, &b.data, thread_id)?, out_shape));
    }
    let out = add_broadcasted(a, b, &out_shape, |x, y| x + y, thread_id)?;
    Ok((out, out_shape))
}

pub fn add_tensor_i64(a: &Tensor<i64>, b: &Tensor<i64>, thread_id: usize) -> Result<(Vec<i64>, Vec<usize>)> {
    let out_shape = if a.shape() == b.shape() {
        a.shape().to_vec()
    } else {
        broadcast_shapes(a.shape(), b.shape())?
    ...
    if a.shape() == b.shape()
        ...
        && is_contiguous(b.shape(), b.strides())
    {
        return Ok((add_i64(&a.data, &b.data, thread_id)?, out_shape));
    }
    let out = add_broadcasted(a, b, &out_shape, |x, y| x + y, thread_id)?;
    Ok((out, out_shape))
}

pub fn add_tensor_u32(a: &Tensor<u32>, b: &Tensor<u32>, thread_id: usize) -> Result<(Vec<u32>, Vec<usize>)> {
    let out_shape = if a.shape() == b.shape() {
        a.shape().to_vec()
    } else {
        broadcast_shapes(a.shape(), b.shape())?
    ...
    if a.shape() == b.shape()
        ...
        && is_contiguous(b.shape(), b.strides())
    {
        return Ok((add_u32(&a.data, &b.data, thread_id)?, out_shape));
    }
    let out = add_broadcasted(a, b, &out_shape, |x, y| x + y, thread_id)?;
    Ok((out, out_shape))
}

pub fn add_tensor_u64(a: &Tensor<u64>, b: &Tensor<u64>, thread_id: usize) -> Result<(Vec<u64>, Vec<usize>)> {
    let out_shape = if a.shape() == b.shape() {
        a.shape().to_vec()
    } else {
        broadcast_shapes(a.shape(), b.shape())?
    ...
    if a.shape() == b.shape()
        ...
        && is_contiguous(b.shape(), b.strides())
    {
        return Ok((add_u64(&a.data, &b.data, thread_id)?, out_shape));
    }
    let out = add_broadcasted(a, b, &out_shape, |x, y| x + y, thread_id)?;
    Ok((out, out_shape))
}

pub fn add_tensor_bool(a: &Tensor<bool>, b: &Tensor<bool>, thread_id: usize) -> Result<(Vec<bool>, Vec<usize>)> {
    let out_shape = if a.shape() == b.shape() {
        a.shape().to_vec()
    } else {
        broadcast_shapes(a.shape(), b.shape())?
    ...
    if a.shape() == b.shape()
        ...
        && is_contiguous(b.shape(), b.strides())
    {
        return Ok((add_bool(&a.data, &b.data, thread_id)?, out_shape));
    }
    let out = add_broadcasted(a, b, &out_shape, |x, y| *x || *y, thread_id)?;
    Ok((out, out_shape))
}

pub fn add_tensor_bitset(
    a: &Tensor<Bitset>,
    b: &Tensor<Bitset>,
    thread_id: usize,
    ...
    if a.shape() == b.shape()
        ...
        && is_contiguous(b.shape(), b.strides())
    {
        return Ok((add_bitset(&a.data, &b.data, thread_id)?, out_shape));
    }
    let out = add_broadcasted(
        a,
        b,
        &out_shape,
        |x, y| Bitset {
