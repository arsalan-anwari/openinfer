...
use super::{abs, add, fill, is_finite, matmul, mul, relu, registry_accumulate};
...
pub fn lookup_kernel_cpu(
    ...
    if matches!(attrs, OpAttrs::Accumulate { .. }) {
        return registry_accumulate::lookup_kernel_cpu_accumulate(
            op,
            output_dtype,
            input_dtypes,
            attrs,
    ...
    match op {
        ...
        OpKind::Matmul => matmul::registry::lookup_kernel_cpu_matmul(output_dtype, input_dtypes, attrs),
        OpKind::IsFinite => is_finite::registry::lookup_kernel_cpu_is_finite(
            output_dtype,
            input_dtypes,
            attrs,
        ),
        OpKind::Fill => fill::registry::lookup_kernel_cpu_fill(output_dtype, input_dtypes, attrs),
    }
}
