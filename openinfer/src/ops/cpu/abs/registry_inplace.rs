...
use super::{
    abs_inplace_bf16, abs_inplace_f16, abs_inplace_f32, abs_inplace_f64, abs_inplace_f8,
    abs_inplace_i16, abs_inplace_i32, abs_inplace_i64, abs_inplace_i8, abs_inplace_i4,
    abs_inplace_i2, abs_inplace_i1,
};
...
pub fn lookup_kernel_cpu_abs_inplace(
    output_dtype: DType,
    input_dtypes: &[DType],
    attrs: &OpAttrs,
    ...
    let kernel: HostInplaceKernel = Box::new(|_attrs, output, _inputs, thread_id| {
        match output {
            ...
            TensorValue::F16(out) => abs_inplace_f16(&mut out.data, thread_id),
            TensorValue::I32(out) => abs_inplace_i32(&mut out.data, thread_id),
            TensorValue::I64(out) => abs_inplace_i64(&mut out.data, thread_id),
            TensorValue::BF16(out) => abs_inplace_bf16(&mut out.data, thread_id),
            TensorValue::F8E5M2(out) => abs_inplace_f8(&mut out.data, thread_id),
            TensorValue::I4(out) => {
                let len = out.numel();
                abs_inplace_i4(&mut out.data, len, thread_id)
            }
            TensorValue::I2(out) => {
                let len = out.numel();
                abs_inplace_i2(&mut out.data, len, thread_id)
            }
            TensorValue::I1(out) => {
                let len = out.numel();
                abs_inplace_i1(&mut out.data, len, thread_id)
            }
