...
pub fn add_accumulate_generic(
    ...
    let strict_shapes = a.shader_setting_bool("strict_shapes").unwrap_or(true);
    if strict_shapes && a.len != b.len {
        return Err(anyhow!("add accumulate shape mismatch"));
    }
    if a.effective_dtype != b.effective_dtype {
        return Err(anyhow!("add accumulate expects matching input dtypes"));
    }
    ...
    let spirv = a
        .spv_bytes_for_target(&target)
        .ok_or_else(|| anyhow!("missing SPIR-V target {} for add accumulate", target))?;
...
pub(crate) fn spv_target_name_add(dtype: DType, attrs: &OpAttrs) -> Result<String> {
    match (dtype, attrs) {
        ...
        | (DType::F64, &OpAttrs::None) => Ok(format!("add_{}", super::dtype_suffix(dtype).unwrap())),
        _ => Err(anyhow!(
...
pub(crate) fn spv_target_name_add_accumulate(
    input_dtype: DType,
    output_dtype: DType,
    attrs: &OpAttrs,
) -> Result<String> {
    match (input_dtype, output_dtype, attrs) {
        ...
        _ => Err(anyhow!(
            "no Vulkan SPIR-V target for add accumulate input {:?}, output {:?}, attrs {:?}",
            input_dtype,
            output_dtype,
            attrs
        )),
