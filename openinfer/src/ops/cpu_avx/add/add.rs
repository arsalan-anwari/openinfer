...
pub fn add_i4_packed(a: &[I4], b: &[I4], logical_len: usize, thread_id: usize) -> Result<Vec<I4>> {
    if a.len() != b.len() {
        return Err(anyhow!("add op shape mismatch"));
    }
    ...
    for idx in 0..logical_len {
        ...
        set_i4_value(&mut out, idx, sum);
    }
    Timer::stop(thread_id);
    Ok(out)
}

pub fn add_i2_packed(a: &[I2], b: &[I2], logical_len: usize, thread_id: usize) -> Result<Vec<I2>> {
    if a.len() != b.len() {
        return Err(anyhow!("add op shape mismatch"));
    }
    ...
    for idx in 0..logical_len {
        ...
        let sum = av.wrapping_add(bv);
        set_i2_value(&mut out, idx, sum);
    }
    Timer::stop(thread_id);
    Ok(out)
}

pub fn add_u4_packed(a: &[U4], b: &[U4], logical_len: usize, thread_id: usize) -> Result<Vec<U4>> {
    if a.len() != b.len() {
        return Err(anyhow!("add op shape mismatch"));
    }
    let storage_len = (logical_len + 1) / 2;
    ...
    for idx in 0..logical_len {
        ...
        let sum = av.wrapping_add(bv);
        set_u4_value(&mut out, idx, sum);
    }
    Timer::stop(thread_id);
    Ok(out)
}

pub fn add_u2_packed(a: &[U2], b: &[U2], logical_len: usize, thread_id: usize) -> Result<Vec<U2>> {
    if a.len() != b.len() {
        return Err(anyhow!("add op shape mismatch"));
    }
    let storage_len = (logical_len + 3) / 4;
