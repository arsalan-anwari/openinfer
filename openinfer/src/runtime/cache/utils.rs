...
#[cfg(test)]
mod tests {
    use super::{increment_scalar, slice_tensor_value};
    use crate::runtime::cache::TensorIndexSelection;
    use crate::tensor::{Tensor, TensorOptions, TensorValue};

    #[test]
    fn increment_scalar_i32() {
        let tensor = Tensor::from_vec_with_opts(vec![1i32], TensorOptions {
            shape: Some(vec![]),
            ..TensorOptions::default()
        }).unwrap();
        let value = TensorValue::I32(tensor);
        let updated = increment_scalar(value, 2, false).unwrap();
        match updated {
            TensorValue::I32(t) => assert_eq!(t.data[0], 3),
            _ => panic!("unexpected dtype"),
        }
    }

    #[test]
    fn slice_tensor_simple() {
        let tensor = Tensor::from_vec_with_opts(vec![1i32, 2, 3, 4], TensorOptions {
            shape: Some(vec![2, 2]),
            ..TensorOptions::default()
        }).unwrap();
        let value = TensorValue::I32(tensor);
        let selections = vec![
            TensorIndexSelection { indices: vec![0], is_scalar: true },
            TensorIndexSelection { indices: vec![1], is_scalar: true },
        ];
        let sliced = slice_tensor_value(&value, &selections).unwrap();
        match sliced {
            TensorValue::I32(t) => assert_eq!(t.data, vec![2]),
            _ => panic!("unexpected dtype"),
        }
    }
}
