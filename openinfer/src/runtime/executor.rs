...
impl Executor {
    ...
    pub fn iterate(&mut self) -> ExecutorIter<'_> {
        ...
        let pending_blocks = build_async_blocks(graph, &entry_block);
        ExecutorIter {
...
pub struct ExecutorIter<'a> {
    exec: *mut Executor,
    frames: Vec<Frame>,
    pending_blocks: VecDeque<BlockFrame>,
            ...
            loop {
                if self.frames.is_empty() {
                    if let Some(block) = self.pending_blocks.pop_front() {
                        self.frames.push(Frame::Block(block));
                    } else {
                        return None;
                    }
                }
...
fn build_async_blocks(graph: &Graph, entry_block: &str) -> VecDeque<BlockFrame> {
    ...
    for name in names {
        ...
        if let Some(block) = graph.blocks.get(&name) {
            pending.push_back(BlockFrame {
                block_name: block.name.clone(),
                nodes: VecDeque::from(block.nodes.clone()),
            });
        }
    }
    pending
