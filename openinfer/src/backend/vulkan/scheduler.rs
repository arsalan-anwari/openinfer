...
impl VulkanScheduler {
    ...
    pub fn schedule_yield(
        ...
        for (block, awaits) in state.block_awaits.clone() {
            if awaits.is_empty() || block == "entry" {
                continue;
            }
...
impl Clone for VulkanScheduler {
    fn clone(&self) -> Self {
        Self {
...
fn run_worker_job(job: WorkerJob) -> WorkerResult {
    let mut updates = HashMap::new();
    let result = Executor::from_snapshot_with_backend(
        job.model,
        job.device,
        job.graph,
        job.backend,
    ...
    .and_then(|mut exec| {
        ...
        match exit {
            BlockExit::Yield(_) | BlockExit::Return => {
