...
Create a kernel under `openinfer-simulator/src/ops/cpu/<category>/<op>/kernel.rs` and wire
...
Add shader sources under `openinfer-simulator/src/ops/vulkan/...` and update SPIR-V
mapping so the kernel is discoverable.

4. Tests and baselines
----------------------

Add tests in `openinfer-simulator/tests/ops` and generate baseline data if the op is part
of baseline suites.
