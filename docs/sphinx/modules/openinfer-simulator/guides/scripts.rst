...
1. Python example script generates `.oinf` files under `openinfer-oinf/res/models`.
...
`sync_models.sh` copies `.oinf` models from `openinfer-oinf/res/models` into
`openinfer-simulator/res/models` for Rust examples and tests.
...
`setup_all.sh` initializes submodules and installs Python dependencies.
`build_all.sh` runs Rust `cargo check` for the Rust submodules.
