...
kernel. The exact file depends on the structure of `openinfer/src/ops/`.

At a minimum you will:

1. Add the op name to `OpKind` in `openinfer/src/graph/types.rs`.
...
The op schemas are parsed from `ops.json` in `openinfer/src/op_defs.rs` at
...
CPU kernels live under `openinfer/src/ops/cpu/`. The typical pattern is:
...
Vulkan ops live under `openinfer/src/ops/vulkan/` and are usually paired with a
